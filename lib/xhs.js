...
const XHS_PAGE_HOSTS = [/(\.|^)xiaohongshu\.com$/i, /(\.|^)xhslink\.com$/i];
const XHS_MEDIA_HOSTS = [/(\.|^)xhscdn\.com$/i];
const XHS_URL_UPGRADE_HOSTS = [...XHS_PAGE_HOSTS, ...XHS_MEDIA_HOSTS];
...
function normalizeAssetUrl(rawUrl) {
  ...
  try {
    ...
    if (
      ...
      XHS_URL_UPGRADE_HOSTS.some((pattern) => pattern.test(parsed.hostname))
    ) {
...
function parseInitialState(blob) {
  ...
  try {
    const ast = acorn.parseExpressionAt(normalized, 0, { ecmaVersion: "latest" });
    const trailing = normalized.slice(ast.end).trim();
    if (trailing) {
      throw new Error("Unsupported note state syntax.");
    }
    return evaluateSafeExpression(ast);
  } catch (error) {
    throw new Error(`Failed to parse note state: ${error.message}`);
  }
}

function evaluateSafeExpression(node) {
  switch (node?.type) {
    case "ObjectExpression": {
      ...
      for (const property of node.properties) {
        ...
        const key = resolvePropertyKey(property.key);
        const value = property.shorthand
          ? evaluateSafeIdentifier(property.key?.name)
          : evaluateSafeExpression(property.value);
        output[key] = value;
      }
      return output;
    }
    case "ArrayExpression":
      return node.elements.map((entry) => {
        ...
        return evaluateSafeExpression(entry);
      });
    case "Literal":
      return node.value;
    case "Identifier":
      return evaluateSafeIdentifier(node.name);
...
function resolvePropertyKey(node) {
...
function evaluateSafeIdentifier(name) {
...
function evaluateUnaryExpression(node) {
  const value = evaluateSafeExpression(node.argument);
...
function validateProxyTarget(rawTarget) {
  try {
    ...
    if (!XHS_MEDIA_HOSTS.some((pattern) => pattern.test(target.hostname))) {
      return null;
    }
